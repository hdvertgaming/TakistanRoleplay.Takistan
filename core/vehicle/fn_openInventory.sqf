...
if((_vehicle getVariable ["trunk_in_use",false]) OR !((_vehicle getVariable ["trunk_user",""])=="")) exitWith {hint localize "STR_MISC_VehInvUse"};
_vehicle setVariable["trunk_in_use",true,true];
_vehicle setVariable["trunk_user",str profileName,true];
...
_vehicle spawn
{
	waitUntil {isNull (findDisplay 3500) OR !(_this getVariable "trunk_user" == profileName) OR !(_this getVariable "trunk_in_use")};
	if (!(_this getVariable "trunk_user" == profileName)) then {
		[[0,format["Duplication Attempt Logged: %1",str profileName]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	};
	_this setVariable["trunk_in_use",false,true];
	_this setVariable["trunk_user","",true];
