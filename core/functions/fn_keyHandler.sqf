...
switch (_code) do {
	...
	case 45: {
		_list = ["landVehicle","Ship"];
		_veh = vehicle player;
		if(KINDOF_ARRAY(_veh,_list) && playerSide in [west,independent] && {vehicle player != player} && {!life_yelp} && {((driver vehicle player) == player)}) then {
			[] spawn {
				life_yelp = true;
				sleep 1.25;
				life_yelp = false;
			...
			if(isNil {_veh GVAR "yelp"}) then {_veh SVAR ["yelp",false,true];};
			if((_veh GVAR "yelp")) then {
				titleText [localize "STR_MISC_YelpOFF","PLAIN"];
				_veh SVAR ["yelp",false,true];
			} else {
				titleText [localize "STR_MISC_YelpON","PLAIN"];
				_veh SVAR ["yelp",true,true];
				if(playerSide == west) then {
					[[_veh],"life_fnc_copYelp",nil,true] call life_fnc_MP;
				} else {
					//I do not have a custom sound for this and I really don't want to go digging for one, when you have a sound uncomment this and change medicSiren.sqf in the medical folder.
					[[_veh],"life_fnc_medicYelp",nil,true] call life_fnc_MP;
				};
			};
		};
	};
